...
namespace Utils
{
    class InitializationParser
    {
        // Examples:
        // Server server_id URL min_delay max_delay -l n [server_id,URL]{n} -p partition_name r [server_id]{r} -p partition_name r [server_id]{r}
        // Client username client_URL script_file -l n [server_id,URL]{n} -p partition_name r [server_id]{r} -p partition_name r [server_id]{r}
        //
        ...
        public List<Tuple<string, string>> getServersConfiguration()
        {
            ...
            for (int i = 0; i < Int32.Parse(serversArray[0]); i++)
            {
                string[] serverAndUrl = serversArray[i+1].Split(",");
        ...
        public List<Tuple<string, List<string>>> getPartitionsConfiguration()
        {
            ...
            for (int i = 1; i < InputLineSplited.Length; i++)
            {
                ...
                Console.WriteLine("PARTITION: " + partitionInput[0]);
                for (int j = 0; j < Int32.Parse(partitionInput[1]); j++)
                {
